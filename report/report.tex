...
\begin{enumerate}
    \item To design and implement a static analysis tool that extracts header info, section details, import lists, and disassembly mnemonics from Portable Executable (PE) and raw assembly source (\texttt{.asm}) files.
...
The project targets Windows executable files, specifically 32-bit and 64-bit binaries in the Portable Executable (\texttt{.exe}, \texttt{.dll}, \texttt{.sys}) format, as well as raw x86/x64 assembly source text (\texttt{.asm}) files. It extracts static features from either the PE structural headers, the disassemblable code segment (\texttt{.text}), or directly from the raw assembly statements in source files. The system does not execute any files, ensuring malware is analyzed inside a secure sandbox containment.
...
\begin{itemize}
    ...
    \item \textbf{Feature Extraction}: Using the \texttt{pefile} library to read PE structure headers, the \texttt{capstone} engine to disassemble compiled bytes within binary code segments into instruction opcodes, or regular-expression text parsing to extract mnemonics, strings, and constants from text-based \texttt{.asm} source files.
    \item \textbf{Heuristic Rule Evaluation}: Feeding the extracted structural features and instruction n-grams into a combined heuristic model that applies scoring weights, evaluates FSM transitions, and adjusts metrics based on file types (binary vs. source) and normal system complexities.
...
\begin{itemize}
    \item \textbf{Binary Disassembly}: For compiled executables, disassembling the \texttt{.text} section using the \texttt{Capstone} engine allows us to inspect instruction sequences and register profiles.
    \item \textbf{Source Text Ingestion}: For raw \texttt{.asm} files, disassembly is unnecessary. Instead, the engine employs text tokenization and regular-expression filters to isolate opcodes, string declarations, and port constants directly.
\end{itemize}
...
\begin{tikzpicture}[node distance=1.8cm, every node/.style={font=\scriptsize}]
    ...
    \node (samples) [decision, right of=dataset, xshift=1.8cm] {Raw PE / .asm / password-protected ZIP};
...
Additionally, qualitative verification of the engine was conducted using raw x86 assembly source code (\texttt{.asm}) samples containing simulated malicious API workflows, decryption loop signatures, and high-risk administrative strings.

\subsection{Engine Optimization and Tuning}
To resolve high false negative and false positive rates observed in the initial heuristic scoring model, six key engine optimizations were designed and implemented in \texttt{engine.py}:
\begin{enumerate}
    ...
    \item \textbf{Assembly Source Code Evaluation}: Integrated a dedicated text-parsing engine for raw assembly source (\texttt{.asm}) files. Bypasses compiled-binary PE checks (such as section properties, raw section size anomalies, entropy thresholds, zero-import tables, and low opcode densities) to prevent false positives and format parser crashes, while preserving instruction ratio, FSM, and string matches.
\end{enumerate}
